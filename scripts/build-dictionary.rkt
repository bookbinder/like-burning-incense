...
(define curdir (string->path "/home/ryan/scores/like-burning-incense/offices/ordinaryTime/notes/"))

(define files (filter
               (lambda (f)
                 (regexp-match? #rx".*.ily$" f))
               (directory-list curdir)))

(define psalm-dict (make-hash))

(for ([f files])
  (let ([shortstem (string-join
                    (take (string-split (path->string f) "-") 3)
                    "-")]
        [psalmnum (second
                   (regexp-match "psalmnum = \"(.*?)\""
                                 (file->string
                                  (string->path (format "~a~a" curdir f)))))])
    (hash-set! psalm-dict shortstem psalmnum)))

(hash-ref psalm-dict "C3-Lauds-2")
